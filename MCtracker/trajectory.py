...
# ---- Noise recording for Kalman filter calibration ----
_noise_buffer = []
_noise_enabled = False

CATEGORY_NAMES = ["car", "pedestrian", "bicycle", "motorcycle", "bus", "trailer", "truck"]


def enable_noise_recording():
    global _noise_enabled
    _noise_enabled = True


def disable_noise_recording():
    global _noise_enabled
    _noise_enabled = False


def get_and_clear_noise():
    global _noise_buffer
    data = list(_noise_buffer)
    _noise_buffer.clear()
    return data


def _record_noise(cat_num, pose_innov, yaw_innov, size_innov, vel_innov):
    """Append a single-frame noise record: (category, dx, dy, dyaw, dl, dw, dvx, dvy)."""
    if not _noise_enabled:
        return
    pi = np.asarray(pose_innov).flatten()
    vi = np.asarray(vel_innov).flatten()
    si = np.asarray(size_innov).flatten()
    _noise_buffer.append((
        int(cat_num),
        float(pi[0]), float(pi[1]),
        float(yaw_innov),
        float(si[0]), float(si[1]),
        float(vi[0]) if len(vi) >= 2 else 0.0,
        float(vi[1]) if len(vi) >= 2 else 0.0,
    ))
...
class Trajectory:
    ...
    def update(self, bbox: BBox, matched_score):
        ...
        if _noise_enabled:
            pose_pred = self.kalman_filter_pose.f(self.kalman_filter_pose.x)
            pose_hx = np.asarray(self.kalman_filter_pose.h(pose_pred)).flatten()
            yaw_pred = self.kalman_filter_yaw.f(self.kalman_filter_yaw.x)
            yaw_hx = np.asarray(self.kalman_filter_yaw.h(yaw_pred)).flatten()
            size_pred = self.kalman_filter_size.f(self.kalman_filter_size.x)
            size_hx = np.asarray(self.kalman_filter_size.h(size_pred)).flatten()
            _record_noise(
                self.category_num,
                np.asarray(pose_mesure).flatten() - pose_hx,
                float(np.asarray(self.get_measure(bbox, filter_flag="yaw")).flatten()[0] - yaw_hx[0]),
                np.asarray(self.get_measure(bbox, filter_flag="size")).flatten() - size_hx,
                np.asarray(pose_mesure).flatten()[2:4] - pose_hx[2:4] if len(pose_hx) >= 4 else [0, 0],
            )
    ...
    def cal_curve_velocity(self):
        if len(self.bboxes) > 2:
            ...
            try:
                popt_x = np.polyfit(x_vals, y_vals_x, 1)
                popt_y = np.polyfit(x_vals, y_vals_y, 1)
                global_velocity_curve = [popt_x[0], popt_y[0]]
            except Exception:
